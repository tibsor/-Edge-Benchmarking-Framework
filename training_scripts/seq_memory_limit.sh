...
do
    ...
    for i in {0..10..1}
        ...
        docker run --name train_mem_bench_test -it -e MEM_LIMIT="$mem_limit" \
        --memory="${mem_limit}m" \
        --cpus="1.0" \
        -v $CURRENT_DIR/host_data:/inference/volume_data \
        --rm inf_bench:latest python3 train_main_ae.py\
        --model_name Sae1d --data_name SEU --data_dir /inference/Mechanical-datasets --normalizetype mean-std --processing_type O_A 
        run_output=$?
