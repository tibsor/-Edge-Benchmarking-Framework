...
#TODO: add docker monitor process kill
     ...
     docker run --name inference_cpu_bench_test -it  -e CPU_QUOTA="$cpu_quota" --memory="1024m" --cpu-period="100000" --cpu-quota="$cpu_quota" \
     -v $CURRENT_DIR/host_data:/inference/volume_data --rm inf_bench:latest \
     python3 train_main.py --model_name MLP --data_name SEU --data_dir /inference/Mechanical-datasets --normalizetype mean-std --processing_type O_A
done
done
# export upper_limit
# export lower_limit
