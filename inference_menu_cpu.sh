...
function CPU_benchmark {
    ...
    #TODO: add docker monitor process kill
        ...
        docker run --rm -it -e CPU_QUOTA="$cpu_quota" --memory="2048m" --cpu-period="100000" --cpu-quota="$cpu_quota" -v $CURRENT_DIR/host_data:/benchmark/volume_data bench_fw:latest python3 inference_main.py --model_name $1 --data_name $2 --normalizetype mean-std --processing_type O_A --max_epoch 10 --middle_epoch 10
        run_output=$?
