...
def inference_plot(dir):
    ...
    for dataset in dataset_folder_list:
        ...
        ax1 = sns.relplot(data=ram_big_boy_df, x='RAM(MB)', y='Time(s)', hue='Model', kind='line', height=7, aspect=5/4,y=1.08)
        ax2 = sns.relplot(data=cpu_big_boy_df, x='CPU Cores', y='Time(s)', hue='Model', kind='line', height=7, aspect=5/4,y=1.08)
        ...
        plt.figure()
        ax3 = sns.barplot(data=low_ram_df, x='Models', y='Minimum RAM').set_title(f"Dataset:{dataset}\nMinimum RAM required for inference")
        fig = ax3.get_figure()
        # ax3.fig.suptitle(f"{dataset}\nMinimum required RAM")
